...
export WP_EMAIL_CONNECTION_STRING=${CUSTOMCONNSTR_WP_EMAIL_CONNECTION_STRING:-$WP_EMAIL_CONNECTION_STRING}
    ...
    if [[ $SETUP_PHPMYADMIN ]] && [[ "$SETUP_PHPMYADMIN" == "true" || "$SETUP_PHPMYADMIN" == "TRUE" || "$SETUP_PHPMYADMIN" == "True" ]]; then
        if [ ! $(grep -c "PHPMYADMIN_INSTALLED" $WORDPRESS_LOCK_FILE) -gt 0 ]; then
            ...
            if [ ! -e "$PHPMYADMIN_HOME/file_session_handler.php" ]; then
                cp "$PHPMYADMIN_SOURCE/file_session_handler.php" "$PHPMYADMIN_HOME/file_session_handler.php"
                update_phpmyadmin_config_file
            fi
           ...
           if [[ $MYSQL_CA_CERT_FILE ]]; then
                if [ ! $(grep -c "MYSQL_CA_CERT_FILE" $PHPMYADMIN_HOME/config.inc.php) -gt 0 ] \
                    || [ ! $(grep -c "^\s*\$cfg\['Servers'\]\[\$i\]\['ssl_ca'\]\s*=\s*getenv('MYSQL_CA_CERT_FILE');" $PHPMYADMIN_HOME/config.inc.php) -gt 0 ]; then
                    update_phpmyadmin_config_file
                fi
            fi
        fi
        ...
        if [ ! $(grep -c "BLOWFISH_SECRET_UPDATED" $WORDPRESS_LOCK_FILE) -gt 0 ]; then
            ...
            if sed -i "s/\(\$cfg\['blowfish_secret'\]\s*=\s*\).*/\1'$BLOWFISH';/" "$PHPMYADMIN_HOME/config.inc.php"; then
...
translate_welcome_content() {
    if [  $(grep -c "WP_LANGUAGE_SETUP_COMPLETED" $WORDPRESS_LOCK_FILE) -gt 0 ] &&  [ ! $(grep -c "WP_TRANSLATE_WELCOME_DATA_COMPLETED" $WORDPRESS_LOCK_FILE) -gt 0 ] &&  [ ! $(grep -c "FIRST_TIME_SETUP_COMPLETED" $WORDPRESS_LOCK_FILE) -gt 0 ]; then
...
setup_wordpress() {
    ...
    if [ $(grep -c "GIT_PULL_COMPLETED" $WORDPRESS_LOCK_FILE) -gt 0 ] &&  [ ! $(grep -c "WORDPRESS_PULL_COMPLETED" $WORDPRESS_LOCK_FILE) -gt 0 ]; then
    ...
    if [ $(grep -c "WORDPRESS_PULL_COMPLETED" $WORDPRESS_LOCK_FILE) -gt 0 ] &&  [ ! $(grep -c "WP_INSTALLATION_COMPLETED" $WORDPRESS_LOCK_FILE) -gt 0 ]; then
    ...
    if [ $(grep -c "WP_INSTALLATION_COMPLETED" $WORDPRESS_LOCK_FILE) -gt 0 ] && [ ! $(grep -c "WP_CONFIG_UPDATED" $WORDPRESS_LOCK_FILE) -gt 0 ]; then
    ...
    if [ $(grep -c "WP_INSTALLATION_COMPLETED" $WORDPRESS_LOCK_FILE) -gt 0 ] && [ ! $(grep -c "SMUSH_PLUGIN_INSTALLED" $WORDPRESS_LOCK_FILE) -gt 0 ]; then
    ...
    if [ $(grep -c "WP_INSTALLATION_COMPLETED" $WORDPRESS_LOCK_FILE) -gt 0 ] && [ ! $(grep -c "EMAIL_PLUGIN_INSTALLED" $WORDPRESS_LOCK_FILE) -gt 0 ] && [[ $WP_EMAIL_CONNECTION_STRING ]] ; then
    ...
    if [ $(grep -c "SMUSH_PLUGIN_INSTALLED" $WORDPRESS_LOCK_FILE) -gt 0 ] && [ ! $(grep -c "SMUSH_PLUGIN_CONFIG_UPDATED" $WORDPRESS_LOCK_FILE) -gt 0 ]; then
    ...
    if [ $(grep -c "WP_INSTALLATION_COMPLETED" $WORDPRESS_LOCK_FILE) -gt 0 ] && [ ! $(grep -c "W3TC_PLUGIN_INSTALLED" $WORDPRESS_LOCK_FILE) -gt 0 ]; then
        #backward compatibility for previous versions that don't have plugin source code in wordpress repo.
        if [ $(grep -c "GIT_PULL_COMPLETED" $WORDPRESS_LOCK_FILE) -gt 0 ]; then
    ...
    if [ $(grep -c "W3TC_PLUGIN_INSTALLED" $WORDPRESS_LOCK_FILE) -gt 0 ] && [ ! $(grep -c "W3TC_PLUGIN_CONFIG_UPDATED" $WORDPRESS_LOCK_FILE) -gt 0 ]; then
    ...
    if [ $(grep -c "W3TC_PLUGIN_CONFIG_UPDATED" $WORDPRESS_LOCK_FILE) -gt 0 ] && [ ! $(grep -c "BLOB_STORAGE_CONFIGURATION_COMPLETE" $WORDPRESS_LOCK_FILE) -gt 0 ] \
    && [ ! $(grep -c "FIRST_TIME_SETUP_COMPLETED" $WORDPRESS_LOCK_FILE) -gt 0 ] && [[ "$IS_BLOB_STORAGE_ENABLED" == "True" ]]; then
    ...
    if [ $(grep -c "W3TC_PLUGIN_CONFIG_UPDATED" $WORDPRESS_LOCK_FILE) -gt 0 ] && [ "$IS_CDN_ENABLED" == "True" ] \
    && [ ! $(grep -c "BLOB_CDN_CONFIGURATION_COMPLETE" $WORDPRESS_LOCK_FILE) -gt 0 ] && [ ! $(grep -c "CDN_CONFIGURATION_COMPLETE" $WORDPRESS_LOCK_FILE) -gt 0 ]; then
        if [ "$IS_BLOB_STORAGE_ENABLED" == "True" ] && [ $(grep -c "BLOB_STORAGE_CONFIGURATION_COMPLETE" $WORDPRESS_LOCK_FILE) -gt 0 ] \
    ...
    if [ $(grep -c "W3TC_PLUGIN_CONFIG_UPDATED" $WORDPRESS_LOCK_FILE) -gt 0 ] && [ "$IS_AFD_ENABLED" == "True" ] \
    && [ ! $(grep -c "BLOB_AFD_CONFIGURATION_COMPLETE" $WORDPRESS_LOCK_FILE) -gt 0 ] && [ ! $(grep -c "AFD_CONFIGURATION_COMPLETE" $WORDPRESS_LOCK_FILE) -gt 0 ]; then
        if [ "$IS_BLOB_STORAGE_ENABLED" == "True" ] && [ $(grep -c "BLOB_STORAGE_CONFIGURATION_COMPLETE" $WORDPRESS_LOCK_FILE) -gt 0 ] \
    ...
    if [  $(grep -c "WP_INSTALLATION_COMPLETED" $WORDPRESS_LOCK_FILE) -gt 0 ] &&  [ ! $(grep -c "WP_LANGUAGE_SETUP_COMPLETED" $WORDPRESS_LOCK_FILE) -gt 0 ] &&  [ ! $(grep -c "FIRST_TIME_SETUP_COMPLETED" $WORDPRESS_LOCK_FILE) -gt 0 ]; then
            if [[ $WORDPRESS_LOCALE_CODE ]] && [[ ! "$WORDPRESS_LOCALE_CODE" == "en_US"  ]]; then
  ...
  # if [ $(grep -c "W3TC_PLUGIN_CONFIG_UPDATED" $WORDPRESS_LOCK_FILE) -gt 0 ] && [ $(grep -c "SMUSH_PLUGIN_CONFIG_UPDATED" $WORDPRESS_LOCK_FILE) -gt 0 ] &&  [ ! $(grep -c "FIRST_TIME_SETUP_COMPLETED" $WORDPRESS_LOCK_FILE) -gt 0 ]; then
    if [ $(grep -c "W3TC_PLUGIN_CONFIG_UPDATED" $WORDPRESS_LOCK_FILE) -gt 0 ] &&  [ ! $(grep -c "FIRST_TIME_SETUP_COMPLETED" $WORDPRESS_LOCK_FILE) -gt 0 ]; then
        echo "FIRST_TIME_SETUP_COMPLETED" >> $WORDPRESS_LOCK_FILE
    fi

    if [[ $(grep -c "WP_INSTALLATION_COMPLETED" $WORDPRESS_LOCK_FILE) -gt 0 ]] && [[ ! $(grep -c "MANAGED_IDENTITY_SETUP_COMPLETED" $WORDPRESS_LOCK_FILE) -gt 0 ]] \
...
initialize_appservice_storage_variable() {
    ...
    if [[ $WEBSITES_ENABLE_APP_SERVICE_STORAGE ]] && [[ "$WEBSITES_ENABLE_APP_SERVICE_STORAGE" == "true" || "$WEBSITES_ENABLE_APP_SERVICE_STORAGE" == "TRUE" || "$WEBSITES_ENABLE_APP_SERVICE_STORAGE" == "True" ]]; then
...
# if [[ $MIGRATION_IN_PROGRESS ]] && [[ "$MIGRATION_IN_PROGRESS" == "true" || "$MIGRATION_IN_PROGRESS" == "TRUE" || "$MIGRATION_IN_PROGRESS" == "True" ]] && [[ $MIGRATE_NEW_DATABASE_NAME ]] && [[ $MIGRATE_MYSQL_DUMP_FILE ]] && [[ $MIGRATE_RETAIN_WP_FEATURES ]] && [ ! $(grep -c "MYSQL_DB_IMPORT_COMPLETED" $MYSQL_IMPORT_STATUSFILE_PATH) -gt 0 ] && [ ! $(grep -c "MYSQL_DB_IMPORT_FAILED" $MYSQL_IMPORT_STATUSFILE_PATH) -gt 0 ]; then
#     service atd start
...
afd_update_site_url() {
    if [[ $AFD_ENABLED ]] && [[ "$AFD_ENABLED" == "true" || "$AFD_ENABLED" == "TRUE" || "$AFD_ENABLED" == "True" ]]; then
        ...
        if [ $(grep -c "BLOB_AFD_CONFIGURATION_COMPLETE" $WORDPRESS_LOCK_FILE) -gt 0 ] || [ $(grep -c "AFD_CONFIGURATION_COMPLETE" $WORDPRESS_LOCK_FILE) -gt 0 ] \
            ...
            if [ -e "$WORDPRESS_HOME/wp-config.php" ]; then
                ...
                if [[ $(grep -c "MULTISITE_CONVERSION_COMPLETED" $WORDPRESS_LOCK_FILE) -gt 0 ]] && [[ $WORDPRESS_MULTISITE_TYPE ]] && [[ "$WORDPRESS_MULTISITE_TYPE" == "subdomain" ]] && [[ $CUSTOM_DOMAIN ]]; then
...
if [[ $(grep -c "WP_INSTALLATION_COMPLETED" $WORDPRESS_LOCK_FILE) -gt 0 ]] && [[ ! $(grep -c "MULTISITE_CONVERSION_COMPLETED" $WORDPRESS_LOCK_FILE) -gt 0 ]] \
    ...
    if [[ "$WORDPRESS_MULTISITE_TYPE" == "subdomain" && "$MULTISITE_DOMAIN" != "$WEBSITE_HOSTNAME" ]] \
        ...
        && wp core multisite-convert ${ADD_SUBDOMAIN_FLAG:+--subdomains} --url=$MULTISITE_DOMAIN --path=$WORDPRESS_HOME --allow-root; then
            ...
            wp site option update fileupload_maxk 51200 --path=$WORDPRESS_HOME --allow-root 2> /dev/null;
...
if [[ $(grep -c "FIRST_TIME_SETUP_COMPLETED" $WORDPRESS_LOCK_FILE) -gt 0 ]] && [[ $WORDPRESS_LOCAL_STORAGE_CACHE_ENABLED ]] && [[ "$WORDPRESS_LOCAL_STORAGE_CACHE_ENABLED" == "1" || "$WORDPRESS_LOCAL_STORAGE_CACHE_ENABLED" == "true" || "$WORDPRESS_LOCAL_STORAGE_CACHE_ENABLED" == "TRUE" || "$WORDPRESS_LOCAL_STORAGE_CACHE_ENABLED" == "True" ]] && [[ "$IS_APPSERVICE_STORAGE_ENABLED" == "True" ]]; then
...
if [[ $IS_APPSERVICE_STORAGE_ENABLED == "True" ]]; then
    ...
    if [[ $SETUP_PHPMYADMIN ]] && [[ "$SETUP_PHPMYADMIN" == "true" || "$SETUP_PHPMYADMIN" == "TRUE" || "$SETUP_PHPMYADMIN" == "True" ]]; then
        if [[ $(grep -c "MULTISITE_CONVERSION_COMPLETED" $WORDPRESS_LOCK_FILE) -gt 0 ]] && [[ $WORDPRESS_MULTISITE_TYPE ]] && [[ "$WORDPRESS_MULTISITE_TYPE" == "subdirectory" ]]; then
            cp /usr/src/nginx/wordpress-subdirectory-multisite-phpmyadmin-server.conf /etc/nginx/conf.d/default.conf
        elif [[ $(grep -c "MULTISITE_CONVERSION_COMPLETED" $WORDPRESS_LOCK_FILE) -gt 0 ]] && [[ $WORDPRESS_MULTISITE_TYPE ]] && [[ "$WORDPRESS_MULTISITE_TYPE" == "subdomain" ]] && [[ "$IS_AFD_ENABLED" == "True" ]] && [[ $CUSTOM_DOMAIN ]]; then
    ...
    #Initialize temporary root to host warning page
    test ! -d /home/site/temp-root && mkdir -p /home/site/temp-root
    cp -r /usr/src/temp-server/* /home/site/temp-root/
...
if [[ ! $DISABLE_WORDPRESS_TELEMETRY  ]] || [[ "$DISABLE_WORDPRESS_TELEMETRY " != "true" && "$DISABLE_WORDPRESS_TELEMETRY " != "TRUE" && "$DISABLE_WORDPRESS_TELEMETRY " != "True" ]]; then
    if [[ "$IS_APPSERVICE_STORAGE_ENABLED" == "True" ]]; then
        ...
        if grep -q "${KUSTO_TELEMETRY_SEARCH_STR}" /etc/supervisord.conf; then
            sed -i "/${KUSTO_TELEMETRY_SEARCH_STR}/r /usr/src/supervisor/kusto-telemetry-supervisord-conf.txt" /etc/supervisord.conf
        fi

        # This is used by telemetry script to avoid log generation immediately after container starts up
